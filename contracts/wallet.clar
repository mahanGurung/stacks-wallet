...
(define-public (lock (new-beneficiary principal) (unlock-at uint) (amount uint))
	(begin
		...
		(asserts! (> unlock-at burn-block-height) err-unlock-in-past)
...
(define-public (claim)
	(begin
		(asserts! (is-eq (some tx-sender) (var-get beneficiary)) err-beneficiary-only)
		(asserts! (>= burn-block-height (var-get unlock-height)) err-unlock-height-not-reached)
        (as-contract? ((with-stx (stx-get-balance tx-sender))) 
            (try! (stx-transfer? (stx-get-balance tx-sender) tx-sender (unwrap-panic (var-get beneficiary))))
        )
	)
)
